...
export default {
  async fetch(request, env) {
    ...
    if (pathname === '/api/charge' && request.method === 'POST') {
      try {
        const body = await request.json().catch(() => ({}));
        // Prefer explicit body user id, but allow fallback to Authorization Bearer token
        let userId = body.userId || body.user_id || body.id || null;
        ...
        // If no userId provided in body, try to resolve from Authorization: Bearer <token>
        if (!userId) {
          ...
          if (token) {
            const userObj = await getUserIdByToken(token, env);
            if (userObj && userObj.id) {
              userId = userObj.id;
              console.log('charge: resolved userId from token=', userId);
            } else {
              // Token present but invalid -> unauthorized if no explicit userId
...
async function getUserIdByToken(token, env){
  ...
  try{
    ...
    // Build select columns dynamically to avoid selecting missing columns on varied schemas
    let colsInfo = null;
    try { colsInfo = await env.DB.prepare("PRAGMA table_info(users)").all(); } catch (e) { colsInfo = null; }
    const presentCols = (colsInfo && colsInfo.results) ? colsInfo.results.map(c => c.name) : [];
    const wantCols = ['id','api_token','kontingent_basis_tts','quota','quota_used','quota_reset_at'];
    const selectCols = wantCols.filter(c => presentCols.includes(c));
    // Ensure at least id and api_token are selected
    if (!selectCols.includes('id')) selectCols.unshift('id');
    if (!selectCols.includes('api_token')) selectCols.splice(1,0,'api_token');
    const selectSql = selectCols.join(',');
    const whereClause = hasTokenCol ? '(token = ? OR api_token = ?)' : '(api_token = ?)';
    const sql = `SELECT ${selectSql} FROM users WHERE ${whereClause} LIMIT 1`;
...
async function checkAndDeductKontingent(env, user_id, amount, type = 'basis') {
  ...
  try {
    ...
    // Select the most recent transaction id — prefer ordering by created_at when present, otherwise fall back to id
    let deductionId = null;
    try {
      const txInfo = await env.DB.prepare("PRAGMA table_info(transactions)").all();
      const txCols = (txInfo && txInfo.results) ? txInfo.results.map(c => c.name) : [];
      if (txCols.includes('created_at')) {
        ...
        deductionId = row2 && row2.id ? row2.id : null;
      } else {
        const row2 = await env.DB.prepare('SELECT id FROM transactions WHERE user_id = ? AND type = ? ORDER BY id DESC LIMIT 1').bind(user_id, txType).first();
        deductionId = row2 && row2.id ? row2.id : null;
      }
    } catch (e) {
      try { const row2 = await env.DB.prepare('SELECT id FROM transactions WHERE user_id = ? AND type = ? ORDER BY id DESC LIMIT 1').bind(user_id, txType).first(); deductionId = row2 && row2.id ? row2.id : null; } catch (e2) { deductionId = null; }
    }
    return { ok: true, deductionId };
  } catch (err) { return { ok: false, error: err && err.message ? err.message : String(err) }; }
}
